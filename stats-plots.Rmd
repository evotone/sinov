...
---
title: "Exploring statistical regularities in the syllable canon of Sino-Vietnamese loanword phonology (Supplementary materials)"
...
```{r, echo=FALSE, out.width = "100%", fig.height = 4.5}

onsv <- on %>%
          mutate(n_SV = replace(n_SV, n_SV<2, NA)) %>%
          ...
          labs(x = "onset", y = "nucleus") +
          ggtitle("Onset-nucleus co-occurrences, SV")
ggplotly(onsv, tooltip = "text")

onnv <- on %>%
          mutate(n_NSV = replace(n_NSV, n_NSV<2, NA)) %>%
          mutate(text = paste0("onset: ", ons, "\n", "nucleus: ", nuc, "\n", "count: ", n_NSV, "\n")) %>%
          ggplot(aes(x = factor(ons, levels = ons_levels),
                     y = factor(nuc, levels = rev(nuc_levels)), 
                     fill = n_NSV, text = text)) +
          geom_tile() +
          scale_fill_viridis(discrete=FALSE) +
          labs(x = "onset", y = "nucleus") +
